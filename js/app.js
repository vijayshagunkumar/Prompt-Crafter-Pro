...
/* ===================================================
   Textarea Expand / Collapse (Final, Stable)
   =================================================== */
(function () {
  const textareas = document.querySelectorAll(
    ".textarea-idea, .output-textarea"
  );

  if (!textareas.length) return;

  let overlay = document.querySelector(".expand-overlay");
  if (!overlay) {
  ...
  textareas.forEach(textarea => {
    ...
    btn.addEventListener("click", () => {
      textarea.classList.contains("textarea-expanded")
        ? close()
        : open();
    });
